...
(() => {
  ...
  /**************************************************
   * API HELPER (รองรับหลายรูปแบบ response)
   **************************************************/
  const api = {
    async call(action, params = {}) {
      ...
      try {
        ...
        try { data = JSON.parse(text); }
          ...
          throw new Error('API ส่งผลลัพธ์ไม่ใช่ JSON (เช็ค URL ว่าถูก exec หรือไม่)');
        }

        // ✅ detect success แบบยืดหยุ่น
        const ok =
          (data.ok === true) ||
          (data.success === true) ||
          (data.status === 'success') ||
        ...
        if (!ok) {
          const msg = data.error || data.message || 'ไม่ทราบสาเหตุจากเซิร์ฟเวอร์';
          throw new Error(msg);
        }

        // ✅ คืนค่าที่ GS นิยมใช้
  ...
  function getCurrentPositionPromise(options = {}) {
    return new Promise((resolve, reject) => {
      if (!('geolocation' in navigator)) {
        reject(new Error('เบราว์เซอร์นี้ไม่รองรับ GPS กรุณาใช้ Chrome/Edge หรือเปิดจากมือถือ'));
        return;
      }
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          if (err.code === err.PERMISSION_DENIED) msg = 'ต้องอนุญาต Location ก่อนบันทึกเวลา (เปิด Location แล้วกด “อนุญาต”)';
          else if (err.code === err.POSITION_UNAVAILABLE) msg = 'ไม่พบสัญญาณตำแหน่ง ลองใหม่ หรือออกนอกอาคาร';
          else if (err.code === err.TIMEOUT) msg = 'ดึงพิกัดนานเกินไป ลองใหม่อีกครั้ง';
  ...
  async function ensureModelsLoaded() {
    ...
    if (line) line.textContent = 'กำลังโหลดโมเดลสแกนใบหน้า (models)...';
    ...
    try {
      ...
      modelsLoaded = true;
      faceReady = true;
      if (line) line.textContent = 'พร้อมสแกนใบหน้าแล้ว ✅';
      return true;
    } catch (err) {
      console.error(err);
      if (line) line.textContent = 'โหลดโมเดลไม่สำเร็จ ❌ ตรวจสอบโฟลเดอร์ models/ และเปิดผ่าน HTTPS';
      faceReady = false;
      return false;
    }
  }
  ...
  async function startCamera(preferDeviceId = '') {
    ...
    const video = $('camVideo');
    if (!video) throw new Error('ไม่พบ <video id="camVideo">');

    video.srcObject = camStream;
    await video.play();

    if (exists('cameraPlaceholder')) $('cameraPlaceholder').style.display = 'none';
    video.style.display = 'block';
    if (exists('camOverlay')) $('camOverlay').style.display = 'block';
    if (exists('scanHud')) $('scanHud').style.display = 'flex';
  ...
  function setScanPills(statusText, foundText) {
    if (exists('scanPillLeft')) $('scanPillLeft').innerHTML = `<strong>สถานะ:</strong> ${statusText}`;
    if (exists('scanPillRight')) $('scanPillRight').innerHTML = `<strong>พบ:</strong> ${foundText}`;
  }
  ...
  async function scanTick() {
    ...
    try {
      ...
      if (!result) {
        stableHit = 0;
        lockedMatch = null;
        if (exists('btnScanSubmit')) $('btnScanSubmit').disabled = true;
      ...
      if (stableHit >= STABLE_MATCH_COUNT) {
        lockedMatch = best;
        if (exists('btnScanSubmit')) $('btnScanSubmit').disabled = false;
  ...
  function startScanLoop() {
    ...
    if (exists('btnStopScan')) $('btnStopScan').disabled = false;
    if (exists('btnStartScan')) $('btnStartScan').disabled = true;
    if (exists('btnScanSubmit')) $('btnScanSubmit').disabled = true;
  ...
  function stopScanLoop() {
    scanning = false;
    if (exists('btnStopScan')) $('btnStopScan').disabled = true;
    if (exists('btnStartScan')) $('btnStartScan').disabled = false;
    if (exists('btnScanSubmit')) $('btnScanSubmit').disabled = true;
  ...
  async function init() {
    ...
    try {
      ...
      try {
        ...
        console.warn('getInitialData failed -> fallback', e);
  ...
  function updateManualEmpInfo() {
    if (!exists('manualCode') || !exists('manualEmpInfo')) return;
  ...
  async function submitManual() {
    const codeEl = $('manualCode');
    const typeEl = $('manualType');
    ...
    if (!codeEl || !typeEl || !fileEl || !resultEl || !errEl || !sucEl || !submitBtn) return;
  ...
  async function submitCheckToday() {
    const codeEl = $('checkCode');
    const resultEl = $('checkResult');
    const errEl = $('checkError');
    if (!codeEl || !resultEl || !errEl) return;
  ...
  function openUrlModal() {
    if (exists('inputApiUrl')) $('inputApiUrl').value = getApiUrl();
    if (exists('urlError')) $('urlError').style.display = 'none';
    showModal('modalUrl');
  }

  function saveUrl() {
    const input = $('inputApiUrl');
    ...
    setApiUrl(url);
    if (exists('apiUrlDisplay')) $('apiUrlDisplay').textContent = 'API: ' + url;
    hideModal('modalUrl');
    init();
  }

  function resetUrl() {
    if (!confirm('ต้องการลบ URL ที่ตั้งค่าไว้หรือไม่?')) return;

    setApiUrl('');
    if (exists('apiUrlDisplay')) $('apiUrlDisplay').textContent = 'ยังไม่ได้ตั้งค่า URL';
    setConnectionStatus(false, 'ยังไม่ได้ตั้งค่า URL');
  ...
  /**************************************************
   * ENROLL FACE (PC + Mobile)
   **************************************************/
  function openEnrollModalPC() {
    if (!exists('modalEnroll')) return;

    if (exists('enrollLoginError')) $('enrollLoginError').style.display = 'none';
    if (exists('enrollDoError')) $('enrollDoError').style.display = 'none';
    if (exists('enrollDoSuccess')) $('enrollDoSuccess').style.display = 'none';

    if (exists('enrollStepLogin')) $('enrollStepLogin').style.display = enrollAuthed ? 'none' : 'block';
    if (exists('enrollStepDo')) $('enrollStepDo').style.display = enrollAuthed ? 'block' : 'none';

    if (exists('enrollInfo')) $('enrollInfo').textContent = 'ถ้ากล้องยังไม่เปิด ให้กด “เปิดกล้อง”';
  ...
  async function enrollLoginPC() {
    ...
    try {
      ...
      if (!res || res.success !== true) throw new Error('เข้าสู่ระบบไม่สำเร็จ');
      enrollAuthed = true;

      if (exists('enrollStepLogin')) $('enrollStepLogin').style.display = 'none';
      if (exists('enrollStepDo')) $('enrollStepDo').style.display = 'block';
    } catch (e) {
  ...
  async function enrollCaptureAndUploadPC() {
    ...
    if (!code) { err.textContent = 'กรุณากรอกรหัสพนักงาน'; err.style.display = 'block'; return; }
    if (!camStream) { err.textContent = 'ยังไม่ได้เปิดกล้อง กรุณากด “เปิดกล้อง”'; err.style.display = 'block'; return; }

    const ok = await ensureModelsLoaded();
    if (!ok) { err.textContent = 'โมเดลไม่พร้อม ตรวจสอบโฟลเดอร์ models/'; err.style.display = 'block'; return; }
    ...
    try {
      ...
      if (!result) throw new Error('ไม่พบใบหน้าในภาพ กรุณาขยับเข้าใกล้/ปรับแสง แล้วลองใหม่');

      const faceData = JSON.stringify(Array.from(result.descriptor));
      const photoBase64 = captureFrameBase64();

      const payload = { code, faceData, photoBase64, fileName: `face-${code}-${Date.now()}.jpg` };
      const res = await api.call('uploadEmployeeFace', payload);

      suc.textContent = `บันทึกใบหน้าสำเร็จ ✅ (${res.code || code})`;
  ...
  function openEnrollMobileSheet() {
    if (!exists('enrollSheetBackdrop')) return;
    setTab('scan');

    if (exists('mEnrollLoginError')) $('mEnrollLoginError').style.display = 'none';
    if (exists('mEnrollDoError')) $('mEnrollDoError').style.display = 'none';
    if (exists('mEnrollDoSuccess')) $('mEnrollDoSuccess').style.display = 'none';

    if (exists('mEnrollStepLogin')) $('mEnrollStepLogin').style.display = enrollAuthed ? 'none' : 'block';
    if (exists('mEnrollStepDo')) $('mEnrollStepDo').style.display = enrollAuthed ? 'block' : 'none';

    if (exists('mEnrollInfo')) $('mEnrollInfo').textContent = 'ถ้ากล้องยังไม่เปิด ให้กด “เปิดกล้อง”';
  ...
  async function enrollLoginMobile() {
    ...
    try {
      ...
      if (exists('mEnrollStepLogin')) $('mEnrollStepLogin').style.display = 'none';
      if (exists('mEnrollStepDo')) $('mEnrollStepDo').style.display = 'block';
    } catch (e) {
      err.textContent = e.message || String(e);
      err.style.display = 'block';
    }
  ...
  async function enrollStartCameraMobile() {
    ...
    try {
      ...
      const deviceId = $('cameraSelect')?.value || '';
      await startCamera(deviceId);
      info.textContent = 'กล้องพร้อมแล้ว ✅ (ดูหน้าบนการ์ดกล้อง แล้วกด “ถ่ายและบันทึกใบหน้า”)';
    } catch (e) {
      console.error(e);
      info.textContent = 'เปิดกล้องไม่สำเร็จ: ' + e.message;
    }
  ...
  function bindMobileSheets() {
    // Menu sheet
    const menu = $('menuSheetBackdrop');
    if (menu) {
      menu.addEventListener('click', (e) => { if (e.target === menu) hideSheet('menuSheetBackdrop'); });
    }

    const enroll = $('enrollSheetBackdrop');
    if (enroll) {
      enroll.addEventListener('click', (e) => { if (e.target === enroll) hideSheet('enrollSheetBackdrop'); });
    }

    on('btnHamburger', 'click', () => showSheet('menuSheetBackdrop'));
    on('btnCloseMenuSheet', 'click', () => hideSheet('menuSheetBackdrop'));

    on('mBtnSetUrl', 'click', () => { hideSheet('menuSheetBackdrop'); openUrlModal(); });
    on('mBtnResetUrl', 'click', () => { hideSheet('menuSheetBackdrop'); resetUrl(); });
    on('mBtnEnrollFace', 'click', () => { hideSheet('menuSheetBackdrop'); openEnrollMobileSheet(); });
    on('mBtnGoAdmin', 'click', () => { hideSheet('menuSheetBackdrop'); window.location.href='admin.html'; });

    on('btnCloseEnrollSheet', 'click', () => hideSheet('enrollSheetBackdrop'));
    on('mBtnEnrollCancelLogin', 'click', () => hideSheet('enrollSheetBackdrop'));
    on('mBtnEnrollClose', 'click', () => hideSheet('enrollSheetBackdrop'));

    on('mBtnEnrollLogin', 'click', enrollLoginMobile);
    on('mBtnEnrollStartCam', 'click', enrollStartCameraMobile);
    on('mBtnEnrollCapture', 'click', enrollCaptureAndUploadMobile);
  ...
  async function boot() {
    ...
    // Header actions (PC)
    on('btnSetUrl', 'click', openUrlModal);
    on('btnSaveUrl', 'click', saveUrl);
    on('btnResetUrl', 'click', resetUrl);
    on('btnGoAdmin', 'click', () => { window.location.href = 'admin.html'; });

    // Manual modal
    on('btnOpenManualModal', 'click', () => {
      if (exists('manualCode')) $('manualCode').value = '';
      if (exists('manualFile')) $('manualFile').value = '';
      if (exists('manualType')) $('manualType').value = 'AUTO';
      if (exists('manualResult')) $('manualResult').value = '';
      if (exists('manualError')) $('manualError').style.display = 'none';
      if (exists('manualSuccess')) $('manualSuccess').style.display = 'none';
      if (exists('manualEmpInfo')) $('manualEmpInfo').textContent = 'กรอกรหัสแล้วระบบจะแสดงชื่อ-สกุล / แผนก / ระดับ';
      showModal('modalManual');
    });
    on('manualCode', 'input', updateManualEmpInfo);
    on('btnManualSubmit', 'click', submitManual);

    // Check today
    on('btnOpenCheckToday', 'click', () => {
      if (exists('checkCode')) $('checkCode').value = '';
      if (exists('checkResult')) $('checkResult').value = '';
      if (exists('checkError')) $('checkError').style.display = 'none';
      showModal('modalCheckToday');
    });
    on('btnCheckTodaySubmit', 'click', submitCheckToday);

    // Today summary
    on('btnRefreshToday', 'click', loadTodaySummary);

    // Enroll button
    on('btnEnrollFace', 'click', () => {
      if (isMobile()) openEnrollMobileSheet();
      else openEnrollModalPC();
    });

    on('btnEnrollLogin', 'click', enrollLoginPC);
    on('btnEnrollStartCam', 'click', enrollStartCameraPC);
    on('btnEnrollCapture', 'click', enrollCaptureAndUploadPC);

    // Camera select change
    on('cameraSelect', 'change', async () => {
      if (!camStream) return;
      try {
    ...
    on('btnStopScan', 'click', () => { stopScanLoop(); stopCamera(); });
    on('btnScanSubmit', 'click', submitFromScan);

    // Pre-list cameras (labels จะขึ้นหลังอนุญาตกล้อง)
    await listCameras();
